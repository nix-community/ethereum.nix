...
let
  ...
  cfg =
    ...
    filterAttrs (_n: v: v.enable) (
      mapAttrs (_: attrByPath [ "backup" ] { enable = false; }) (findEnabled config.services.ethereum)
    );
  ...
  mkBackupService =
    ...
    lib.nameValuePair "${name}-backup" {
      ...
      environment = {
        ...
        SNAPSHOT_ENABLE = if cfg.btrfs.enable then "1" else "0";
      ...
      // optionalAttrs (restic.rcloneOptions != null) (
        mapAttrs' (
      ...
      // optionalAttrs (restic.rcloneConfig != null) (
        mapAttrs' (
...
in
{
  config.systemd = {
    ...
    tmpfiles.rules =
      with lib;
      map (name: "v /var/lib/private/${name}") (
        builtins.attrNames (filterAttrs (_: v: v.btrfs.enable) cfg)
      );

    services = backupServices // metadataServices // clientServiceCfgs;

    timers = backupTimers // metadataTimers;
  };
}
