...
let
  ...
  defaultArgReducer = value: if (isList value) then concatStringsSep "," value else toString value;
  ...
  defaultArgFormatter =
    ...
    in
    if (opt.type == types.bool) then (if value then "${arg}" else "") else "${arg} ${argReducer value}";
  ...
  mkArgs =
    ...
    collect (v: (isString v) && v != "") (
      mapAttrsRecursiveCond (as: !isOption as) (
        path: opt:
        mkArg {
          inherit
            path
  ...
  findEnabled =
    ...
    let
      op =
        ...
        in
        if (attrByPath [ "enable" ] false val) then
          (sum // { "${pathStr}" = val; })
        else if ((isAttrs val) && !(val ? enable) && !(isDerivation val)) then
      ...
      recurse =
        sum: path: val:
        foldl' (sum: key: op sum (path ++ [ key ]) val.${key}) sum (attrNames val);
    in
    recurse { } [ ] tree;
}
