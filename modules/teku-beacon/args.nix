...
with lib; {
  network = mkOption {
    type = types.enum ["mainnet" "holesky" "hoodi" "ephemery" "sepolia" "minimal" "gnosis" "chiado" "lukso"];
    default = name;
    defaultText = "name";
  ...
  data-storage-mode = mkOption {
    type = types.nullOr (types.enum ["archive" "minimal" "prune"]);
    default = "minimal";
